...
\chapter{Bipolární tranzistory}\label{es:IchapV}
  \section{Všeobecné poznatky}\label{es:IchapVsecI}
    ...
    Střídavé zesilovače mohou naproti tomu zesilovat střídavá napětí o frekvenci několika \unit{\Hz}
    až do několika \unit{\GHz} (podle typu tranzistoru a dimenzování zapojení), avšak nemusejí
  ...
  \section{Základní princip}\label{es:IchapVsecII}
    \subsection{Princip funkce NPN a PNP tranzistorů}\label{es:IchapVsecIIssecI}
      ...
      tranzistorů má velikost \(U_{BE} \approx \qty{0.7}{\V}\), tj. stejné, jako je difuzní napětí
      ...
      jenž protéká zmenšenou závěrnou vrstvou. Např. při napětí \(U_{BE} \approx \qty{0.7}{\V}\)
      protéká bází proud \(I_B = \qty{1}{\mA}\) a kolektorem proud \(I_C \approx \qty{100}{\mA}\).
      Jestliže nyní nepatrné zvětšíme \(U_{BE}\), vzroste proud báze např. na \(I_B = \qty{2}{\mA}\),
      čímž k závěrné vrstvě doputuje více nositelů náboje z emitoru. Tím se závěrná vrstva ještě
      více zmenší a kolektorový proud vzroste např. na \(I_C \approx \qty{200}{\mA}\). Naopak při
      ...
      napěťový dělič pro napájecí napětí \(U_{CC} = \qty{+10}{\V}\). Při \(I_C = \qty{5}{\mA}\) a
      \(R_C = \qty{1}{\kohm}\) vzniká na kolektorovém rezistoru úbytek napětí
      \begin{equation*}
        U_{RC} = \qty{5e-3}{\mA}\cdot\qty{1}{\kohm} = \qty{5}{\V}
      \end{equation*}
      ...
      \begin{equation*}
        U_{CE} = U_{CC} - U_{RC} = \qty{10}{\V} - \qty{5}{\V} = \qty{5}{\V}
      \end{equation*}
      Jestliže se napětí \(U_{BE}\) vlivem střídavého napětí z připojeného generátoru v daný okamžik
      zvětší např. na \(U_{BE} = \qty{0.71}{\V}\), vzroste proud báze, který způsobí zvětšení
      kolektorového proudu, který, dejme tomu, vzroste z \(I_C = \qty{5}{\mA}\) na \(I_C =
      \qty{6.5}{\mA}\). Tento zvětšený kolektorový proud vyvolá na kolektorovém rezistoru zvětšený
      úbytek napětí
      \begin{equation*}
        U_{RC} = I_C\cdot R_C = \qty{6.5}{\mA}\cdot\qty{1}{\kohm} = \qty{6.5}{\V}
      \end{equation*}
      Tím napětí \(U_{CE}\) klesne na hodnotu \(U_{CE} = \qty{3.5}{\V}\). Při záporné půlvlně napětí
      ...
      připojený na bázi tranzistoru, způsobí změnu proudu báze \(\Delta I_B = \qty{20}{\micro\A}\),
      která vyvolá změnu kolektorového proudu \(\Delta I_C = \qty{3}{\mA}\).
      ...
      \(R_C\). V příkladu způsobí vstupní střídavé napětí \(\Delta U_{BE} = \qty{-0.02}{\V}\) změnu
      výstupního střídavého napětí \(U_{CE} = \qty{-3}{\V}\). V tomto případě je tedy napěťové
      zesílení
      \begin{equation*}
