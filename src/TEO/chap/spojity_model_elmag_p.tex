...
\chapter{Spojité matematické modely polí}
      ...
      \begin{subequations}
        \begin{alignat}{3}
          ...
          \oint_{\mathcal{C}}\vec{E} \dd{\vec{l}}, &= -\der{\Phi}{t}
           ...
           \int_{\mathcal{S}}\vec{D} \dd{\vec{S}}, &= Q \qquad\quad\;   
                                       \quad &&\diver{D} &&=\rho_V                             \\
           \int_{\mathcal{S}}\vec{B} \dd{\vec{S}}, &= 0 \qquad
                                       \quad &&\diver{B}&& =0                                  \\
    ...
    \subsection{Elektrický proud v kovových vodičích}\label{TEMP:kap_elproud_jev}
      ...
      \begin{minipage}{\linewidth}
        ...
        \begin{minipage}{0.35\linewidth}
          \begin{figure}[H]
            \includegraphics[width=1\linewidth]{plocha_S.pdf}
            \caption[Rovinná plocha $S$.]{Rovinná plocha $S = S_0\cos\alpha$}
            \label{TEMP:fig_plocha_S}
          \end{figure} 
        \end{minipage}
      \end{minipage}
      \noindent a o směru vektoru driftové rychlosti nositelů kladného náboje. Pro případ nositelů 
      volného náboje - elektronů má proudová hustota opačný směr než driftová rychlost $v_d$ (obr.
  ...
  \section{Stacionární magnetické pole}
    ...
    ukazují směr vektoru $\vec{H}$, obr. \ref{temp:fig_pravidlo_ruka}.

    \begin{figure}[ht!]
      \centering
      \subcaptionbox{\label{temp:fig_pravidlo_ruka}}{\luafigure[0.4]{pravidlo_prave_ruky.pdf}}
      \hspace{1cm}
      \subcaptionbox{\label{temp:fig_pravidlo_sroub}}{\luafigure[0.4]{pravidlo_pravotociveho_sroubu.pdf}}
      \caption[Pravidlo pravé ruku a pravotočivého šroubu]{Určení směru vektoru $\vec{H}$: a)
    ...
    \begin{figure}[ht!]
      \centering
      \subcaptionbox{\label{TEMP:fig_ind_c_kruh_z}}{\luafigure[0.4]{mag_ind_cary_kruh_z.pdf}}
      \hspace{1cm}
      \subcaptionbox{\label{TEMP:fig_1MR_pic}}{\luafigure[0.45]{1MR_pic.pdf}}
      \caption{a) Indukční čáry kruhového závitu; b) K zákonu celkového proudu}
      \label{temp:fig_ind_cary}
    \end{figure}
    ...
    \begin{figure}[ht!]
      \centering
      \subcaptionbox{$\oint\vec{H}\cdot \dd{\vec{l}} = 0$ \label{TEMP:fig_mag_sprazeny_proud1}}
        {\luafigure[0.3]{mag_sprazeny_proud1.pdf}}
      \subcaptionbox{$\oint\vec{H}\cdot \dd{\vec{l}} = 0$ \label{TEMP:fig_mag_sprazeny_proud2}}
        {\luafigure[0.3]{mag_sprazeny_proud2.pdf}}
      \subcaptionbox{$\oint\vec{H}\cdot \dd{\vec{l}} = 3I$ \label{TEMP:fig_mag_sprazeny_proud3}}
        {\luafigure[0.3]{mag_sprazeny_proud3.pdf}}             
    ...
    platí podmínka
    \begin{alignat}{3}
      & H &&= \text{konst}, \qquad \alpha &&= \text{konst},  \label{TEMP:eq_H_alpha_konst}  \\
      \shortintertext{speciálně}  
      & H &&= \text{konst}, \qquad \alpha &&= 0.             \label{TEMP:eq_alpha_0}
    \end{alignat}
    ...
    \begin{figure}[ht!]
      \centering
      \includegraphics[width=0.5\linewidth]{mag_pole_vodic_I_konst.pdf}
    ...
    \begin{figure}[ht!]
      \centering
      \includegraphics[width=0.6\linewidth]{teo_fig021.pdf}
    ...
    \begin{equation}\label{TEMP:eq_mag_napeti}
      \int_{M(l)}^N\vec{H}\dd{\vec{l}} = U_{m_{MN}} \qquad [A]
    \end{equation}       
    ...
    \subsection{Magnetické pole elektrického proudu v diferenciálním tvaru}
      ...
      \begin{align}\label{TEMP:eq_amp_z4}
        \textsf{rot}_x\mathbf{H} 
          &= \frac{1}{dydz}\oint_{\mathcal{c}}\vec{H_c}\dd{\vec{l}} 
           = \frac{1}{dydz}\cdot\Biggl[\Biggr.                       \nonumber \\
          &= \Biggl.\left[H_ydy + 
                           \left(H_z + \pder{Hz}{y}dy\right)dz\right]      
           - \left[\left(H_y - \pder{Hy}{z}dz\right)dy - H_zdz\right]\Biggr]           \nonumber \\
          &= \left[\pder{Hz}{y}dydz - \pder{Hy}{z}dydz\right]  
           = \pder{Hz}{y} - \pder{Hy}{z} = J_z
      \end{align}       
      
      \luagraphic[0.8]{teo_fig062.pdf}{K odvození pojmu \(\textsf{rot}_z\mathbf{H}\)}{teo:fig062}
         
      tedy dostáváme
      \begin{subequations}
        \begin{align}\label{TEMP:eq_amp_z5}
          \textsf{rot}_x\mathbf{H} &= \pder{H_z}{y} - \pder{H_y}{z} = J_x       \\
          \textsf{rot}_y\mathbf{H} &= \pder{H_x}{z} - \pder{H_z}{x} = J_y       \\
          \textsf{rot}_z\mathbf{H} &= \pder{H_y}{x} - \pder{H_x}{y} = J_z            
        \end{align}    
    \end{subequations}    
      ...
      \begin{equation*} % \label{TEMP:eq_amp_z6}
        \textsf{rot}\mathbf{H} 
           = \vec{i}\,\textsf{rot}_x\mathbf{H} + 
             \vec{j}\,\textsf{rot}_y\mathbf{H} +
             \vec{k}\,\textsf{rot}_z\mathbf{H}
     \end{equation*}
     \begin{align*}
      ...
      \begin{equation*} %\label{TEMP:eq_amp_z7}
        \rot{H} = \nabla\times\vec{H}                                                    
                = \left(\vec{i}\,\pder{ }{x} + 
                  \vec{j}\,\pder{ }{y} + \vec{k}\,\pder{ }{z}\right)\times(\vec{i}\,H_x +
                  \vec{j}\,H_y + \vec{k}\,H_z)
      \end{equation*}
