...
\chapter{Kmitočtové filtry}
  ...
  \section{Kmitočtové filtry a jejich použití}
    ...
    \begin{figure*}[ht!]
      ...
      \caption{Příklad selekce kmitočtových složek signálu filtrem typu horní propust pro potlačení
                nízkofrekvenční rušivé složky (např. kmitočet sítě \qty{50}{\Hz)}
  ...
  \section{Popis přenosových vlastností filtrů, jejich charakteristiky}
    ...
    \subsection{Rozklad a způsoby vyjádření přenosové funkce filtru}
      ...
      \begin{tcnote}
        Prakticky se používá kmitočet \(f\) v jednotkách [\unit{\Hz}], ale v některých teoretických
        ...
        přenosovou funkci 2. řádu (1.12) s rezonančním kmitočtem \(F_0 = \qty{1}{\kHz}\) se
        zobrazením v kmitočtových charakteristikách osou v [\unit{\Hz}], musíme jako parametr
        \(b_0\) dosadit hodnotu \num{39478417.6} namisto hodnoty \num{e6}, což je dosti
        nepraktické. Přenosovou funkci s proměnou kmitočtu \(f\) [\unit{Hz}] Ize vyjádřit ve tvaru 
        \begin{equation*}
          \mathbb{K}(j\omega) 
