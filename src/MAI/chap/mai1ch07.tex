...
\chapter{Poznáváme funkci z její derivace - neurčitý integrál}
  ...
  \twocolumn[\section{Metody určení primitivní funkce}]
    ...
    \twocolumn[\subsection{Integrace ryze lomené racionální funkce rozkladem na parciální zlomky}]
      Některé příklady v předchozím odstavci, (viz např. \ref{MA:int_ex_01}) jsme dělením čitatele
      ...
      \begin{gather*}
        \frac{M}{2}\underbrace{\frac{2x + p}{(x^2+px+q)^k}}_{I_1} + 
        \left(b - \frac{MN}{2}\right)\underbrace{\frac{1}{(x^2+px+q)^k}}_{I_2}
      \end{gather*} 
      ...
      \begin{equation*}
        \boxed{I_1=\int{\frac{(2x+p)}{(x^2+px+q)^k}\dd{x}}=\int\frac{1}{u^k}\dd{u}}\, ,                   
      \end{equation*}  
      pro \(k\neq1\) dostaneme
      \begin{equation*}
      ...
      Zbývá vyřešit nejpracnější problém, určit primitivní funkci ke zlomku \((t^2+1)^{-k}\). 
      
      Pro \(k=1\) je to velmi jednoduché, neboť podle tabulkového integrálu \ref{mai:eq121}
      dostaneme
      ...
      V případě \(k = 2\) \cite[p.~141]{Musilova2009MA1}. vyjádříme integrál 
      \begin{equation*}
      ...
      \begin{equation}\label{mai:eq165}
        I_k = \int\dfrac{1}{(1+u^2)^k}\dd{u} 
