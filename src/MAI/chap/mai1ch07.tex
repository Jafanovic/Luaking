...
\chapter{Poznáváme funkci z její derivace — neurčitý integrál}
  ...
  \section{Motivace}
    ...
    \begin{mdframed}[style=mdmathdef] 
      ...
      \textbf{primitivní} funkcí k funkci \(f(x)\) na intervalu \(J\) právě když, pro všechna \(x\in
      J\) je \(F'(x) = f(x)\). 
    ...
    \begin{align}
      \int[f(x)\pm g(x)]\dd{x} &= \int f(x)\dd{x} + \int g(x)\dd{x} \label{MA1:eq_int10} \\
  ...
  \section{Tabulka neurčitých integrálů}\label{MA:chap_tabINT}
    ...
    \begin{flalign}
      \hline
      & \int 0\dd{x} = c                                        &      \label{MA:baseInt01}     \\
      & \int a\dd{x} = ax+c                                     &      \label{MA:baseInt02}     \\
      ...
      & \int e^x \dd{x}       = e^x+c                          &       \label{MA:baseInt05}    \\
      & \int\ln x\dd{x}       = 
          x\ln x - x + c \hspace{1ex}\forall x>0               &       \label{MA:baseInt06}    \\
      & \int a^x \dd{x}     =
        \frac{a^x}{\ln a}+c 
        \hspace{1ex}\forall a>0,\,a\neq1                       &       \label{MA:baseInt07}    \\
      & \int \sin x \dd{x}  = -\cos x                          &       \label{MA:baseInt08}    \\
      & \int \cos x \dd{x}  =  \sin x                          &       \label{MA:baseInt09}    \\
      & \int\frac{1}{\cos^2x}\dd{x} =  \tan x+c 
          \hspace{1ex}\forall x\neq(2k+1)\pi,\,k\in\naturalset &       \label{MA:baseInt10}    \\ 
    \end{flalign}

    \begin{flalign}          
      & \int\frac{1}{\sin^2x}\dd{x}     =  -\cotg x+c
     ...
     & \int\cosh\dd{x} = \sinh x + c                           &       \label{MA:baseInt13}    \\
     & \int\sinh\dd{x} = \cosh x + c                           &       \label{MA:baseInt14}    \\
     & \int\frac{1}{1+x^2}\dd{x} = \arctan x + c               &       \label{MA:baseInt15}    \\
    ...
    \begin{flalign}
      ...
      & \int\tan x \dd{x}   = \ln\abs{\sec x} + c               &      \label{MA:baseInt21}    \\
      & \int\sec x \dd{x}   = \ln\abs{\sec x + \tan x} + c      &      \label{MA:baseInt22}    \\
      & \int\sec^2 x \dd{x} = \tan x + c                        &      \label{MA:baseInt23}    \\
      & \int\sec x\tan x \dd{x} = \sec x + c                    &      \label{MA:baseInt24}    \\
      & \int\frac{a}{a^2+x^2}\dd{x} = \tan^{-1}\frac{x}{a}      &      \label{MA:baseInt25}    \\
