\begin{fyzexam}{Při skutečném uspořádání pokusu podle náčrtu na obrázku \ref{fyz:fig0903}, nejsou
  ...
  \captionof{figure}{Interferometr Michelsona a Morleyho, namontovaný na kamenné desce, která se
              vznáší v kruhovém žlabu naplněný rtutí. Kredit: Wikipedia heslo: \wikiMichelMorlexp
              \label{fyz:fig0902}} 
  \par}
  \vspace{1em}
  ...
  \captionof{figure}{Jediný paprsek světla \((a)\) je rozdělen do kolmých cest polopropustným
              ...
              zrcátkem \((e)\) na protějších rozích kamenné desky. Průhledná skleněnaá destička
              má zajišťovat, aby oba paprsky procházely stejně silnou vrstvou vzduchu a skla. Poté
              se oba paprsky znovu spojí dříve, než vstoupí do dalekohledu \((f)\).  
  \cite[s.~18]{Beiser1975}
  \label{fyz:fig0903}} \par}
  \vspace{1em}

  Podle rovnic (\ref{fyz:eq186}) a (\ref{fyz:eq187}) je rozdíl časů u obou drah následkem etherového
  proudu
  \begin{equation*}
  ...
  \qty{3e4}{\metre\per\second}, a \(V\) rychlost světla \(c\), kde \(c =
  \qty{3e8}{\metre\per\second}\). Tudíž
  \begin{equation*}
  ...
  \end{equation*}    
  Ve skutečném uspořádání pokusu dosáhli Michelson a Morley s využitím několikanásobných odrazů (viz
  obr. \ref{fyz:fig0903}) efektivní délky \(L\) kolem \qty{10}{\meter}, vlnová délka použitého světla
  byla asi \qty{500}{\nano\meter}. Očekávané posunutí interferenčního obrazu na každé dráze při
  otočení přístroje o \ang{90} je tudíž 
  \begin{align*}
    n  = \dfrac{Lv^2}{\lambda c^2} 
      &= \dfrac{\qty{10}{\meter}\cdot(\qty{3e4}{\meter\per\second})^2}
              {\qty{5e-7}{\meter}\cdot(\qty{3e8}{\meter\per\second})^2}     \\
      &= 0.2\,\text{proužku}
  \end{align*}
  {\centering
  \captionsetup{type=figure}
